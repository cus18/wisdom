...
//	public static final String secret="d4624c36b6795d1d99dcf0547af5443d";
    ...
    public static String getWechatToken() {
        try {
            ...
            //调用获取微信服务器 IP 接口测试 Token 是否有效
            String result = HttpRequestUtil.get("https://api.weixin.qq.com/cgi-bin/getcallbackip?access_token=" + token);
    ...
    public String processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (msgType.equals(MessageUtil.REQ_MESSAGE_TYPE_EVENT)) {
            ...
            //获取用户位置
                ...
                respMessage = processGetLocationEvent(xmlEntity, request);
            }
        }
        return respMessage;
    }
...
//		}
    ...
    private String processGetLocationEvent(ReceiveXmlEntity xmlEntity, HttpServletRequest request) {
        ...
        if (weChatAttentionMapper.updateWeChatUserLocation(weChatUserLocation) == 0) {
            weChatAttentionMapper.insertWeChatUserLocation(weChatUserLocation);
        }

        return "success";
    }

    public String getWeChatUserLocation(String openid){
        try {
            WeChatUserLocation weChatUserLocation=weChatAttentionMapper.getWeChatUserLocation(openid);
            String result = HttpRequestUtil.get("http://apis.map.qq.com/ws/geocoder/v1/?location="+weChatUserLocation.getLatitude()+","+weChatUserLocation.getLongitude()+"&coord_type=1&key=L6WBZ-WVAW6-BFJSL-MEX6R-4OMX2-INFYN");
            Map<String,Object> map = JSONObject.parseObject(result, Map.class);
            map.get("status").toString();
            if(map.get("status").toString().equals("0")){
                Map<String,Object> map2 = JSONObject.parseObject(map.get("result").toString(), Map.class);
                Map<String,String> map3= JSONObject.parseObject(map2.get("formatted_addresses").toString(), Map.class);
                return map3.get("recommend").toString();
            }else{
                return "网络错误";
            }
        } catch (IOException e) {
